...
type RuneliteLinkMutationResponse = {
  ok: boolean;
  message?: string;
...
export default function ProfilePage() {
  ...
  const [runeliteLink, setRuneliteLink] = useState<RuneliteLinkStatus | null>(null);
  ...
  const [unlinkingRunelite, setUnlinkingRunelite] = useState(false);
  ...
  async function refreshLinkedState() {
    const [womData, shellData, runeliteData] = await Promise.all([
      getJSON<WomMeData>('/api/wom/me').catch(() => null),
      getJSON<ShellData>('/api/site-shell').catch(() => null),
      getJSON<RuneliteLinkStatus>('/api/profile/runelite-link').catch(() => null),
    ]);
    setWomMe(womData);
    if (shellData) setShell(shellData);
    setRuneliteLink(runeliteData);
  }
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [womData, runeliteData] = await Promise.all([
          getJSON<WomMeData>('/api/wom/me').catch(() => null),
          getJSON<RuneliteLinkStatus>('/api/profile/runelite-link').catch(() => null),
        ]);
        setWomMe(womData);
  ...
  async function handleRuneliteUnlink() {
    setUnlinkingRunelite(true);
    setLinkResult(null);
    try {
      const payload = await getJSON<RuneliteLinkMutationResponse>('/api/profile/runelite-link', {
        method: 'DELETE',
      });
      setRuneliteLink(payload.link);
      setLinkResult({ ok: true, message: normalizeHallProfileCopy(payload.message ?? 'RuneLite account unlinked.') });
      setWomMe(null);
      await refreshLinkedState();
    } catch (nextError) {
      setLinkResult({
        ok: false,
        message: nextError instanceof Error ? normalizeHallProfileCopy(nextError.message) : 'Failed to unlink RuneLite account.',
      });
    } finally {
      setUnlinkingRunelite(false);
    }
  }
  ...
  const hasRuneliteLink = Boolean(runeliteLink?.linked);
  const verificationTimestamp = runeliteLink?.lastVerifiedAt ?? wom?.verifiedAt ?? null;
  const isPluginVerified = Boolean(runeliteLink?.linked || wom?.verifiedAt);
  const profileSummary = runeliteLink?.linked
    ? 'Your Hall identity is linked through Ghosted Authenticator.'
    : wom?.linked
      ? 'Wise Old Man is linked, and Ghosted Authenticator is ready to upgrade it to a verified plugin claim.'
      : 'Link your OSRS identity to load your clan status in the Hall.';
        ...
        <>
              ...
              body={(
                <div className="app-stack">
                  ...
                  <div className={styles.linkStatus}>
                    <strong>
                      {hasRuneliteLink
                        ? 'Ghosted Authenticator is linked and verified.'
                        : wom?.linked
                          ? 'Wise Old Man is linked, and plugin verification is ready.'
                          : 'OSRS identity still needs to be linked.'}
                    </strong>
                    <span>
                      {hasRuneliteLink
                        ? `Ghosted is verifying ${runeliteLink?.username ?? womMe?.player?.displayName ?? wom?.displayName ?? wom?.username ?? 'your linked account'} through the Plugin Hub-safe pairing flow.`
                        : wom?.linked
                          ? `Ghosted is already tracking ${womMe?.player?.displayName ?? wom?.displayName ?? wom?.username ?? 'your linked account'} across the Hall. Install Ghosted Authenticator in RuneLite to upgrade it to a verified claim.`
                          : 'Link the RuneScape username your clan tracks so your Hall profile matches live clan data, then finish plugin verification from RuneLite.'}
                    </span>
                  </div>
                  ...
                  <MetricGrid
                    items={[
                      ...
                      ['RuneLite link', hasRuneliteLink ? 'Verified' : 'Not linked'],
                      ['Claim source', claimSourceLabel(currentClaimSource)],
                      ['Verification', isPluginVerified ? 'Verified' : (wom?.linked ? 'Ready to verify' : '-')],
                  ...
                  <div className={styles.claimControls}>
                    <div className={styles.claimHeader}>
                      <strong>Verified member name</strong>
                    ...
                    <div className={styles.claimFacts}>
                      ...
                      <span>
                        Verification:{' '}
                        <strong>{verificationTimestamp ? `Verified ${formatDate(verificationTimestamp)}` : (wom?.linked ? 'Ready for plugin verification' : 'Not available')}</strong>
                      </span>
                    </div>
                    <div className={styles.claimActions}>
                      <span className={styles.claimHint}>
                        Ghosted Authenticator is the preferred verification path. Install it from the RuneLite Plugin Hub,
                        choose <strong>Link Current Account</strong>, then approve the browser pairing here.
                      </span>
                    </div>
                    {hasRuneliteLink ? (
                      <div className={styles.claimActions}>
                        <button
                          ...
                          disabled={unlinkingRunelite}
                          onClick={() => void handleRuneliteUnlink()}
                        >
                          {unlinkingRunelite ? 'Unlinking...' : 'Unlink Ghosted Authenticator'}
                        </button>
                        <span className={styles.claimHint}>
                          Linked account hash: <strong>{runeliteLink?.accountHash ?? 'Unknown'}</strong>{' '}
                          {runeliteLink?.linkedAt ? `| Linked ${formatDate(runeliteLink.linkedAt)}` : ''}
                        </span>
                      </div>
                    ) : null}
                  </div>
                  ...
                  {hasRuneliteLink ? (
                    <div className="app-stack app-stack--compact">
                      <p className="app-panel-note">
                        Ghosted Authenticator has verified{' '}
                        <strong className="profile-identity__inline-strong">
                          {runeliteLink?.username ?? womMe?.player?.displayName ?? wom?.displayName ?? wom?.username ?? 'your OSRS account'}
                        </strong>
                        {' '}for your public Ghosted identity.
                      </p>
                    </div>
                  ) : wom?.linked ? (
                    <div className="app-stack app-stack--compact">
                      ...
                      <p className="app-panel-note">
                        Manual Wise Old Man linking still works as a fallback, but plugin verification is now the preferred path.
                      </p>
                      <div className="app-inline-actions">
                        ...
                        >
                          {linking ? 'Unlinking...' : 'Remove manual Wise Old Man link'}
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div className="app-stack">
                      <p className="app-panel-note">
                        Use Ghosted Authenticator first if you can. The form below is still available as a compatibility fallback.
                      </p>
          ...
          <StatStrip
            ...
            stats={[
              ...
              { label: 'Plugin verification', value: hasRuneliteLink ? 'Verified' : 'Pending' },
