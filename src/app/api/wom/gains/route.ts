...
import { DEFAULT_WOM_HISCORE_METRIC, DEFAULT_WOM_PERIOD, womGroupGainsPayload } from '@/lib/server/wom';
...
export const GET = withRouteErrorHandling(async (request: Request) => {
  ...
  const period = url.searchParams.get('period') ?? DEFAULT_WOM_PERIOD;
  const limit = Number.parseInt(url.searchParams.get('limit') ?? '10', 10) || 10;
  return NextResponse.json(await womGroupGainsPayload(getDatabase(), metric, period, limit));
});
